...
class ActTestSpecParserTest : FreeSpec({
    ...
    "captures job ID" {
        ...
        parser.parse(specFile) shouldBeEqual
            actSpec(
                name = "spec running a single job",
                job = "awesome-job",
            )
    ...
    "handles valid event payload file" {
        val payloadFile =
            specsRoot.resolve("event_payload.json").apply {
                this.createNewFile()
            }
        specFile.writeText(
            """
    ...
    withData(
        ...
        val eventFile =
            specsRoot.resolve("values.json").apply {
                this.createNewFile()
            }
        specFile.writeText(
            """
        ...
        val expectedSpec =
            when (inputType) {
                ...
                "inputs" -> actSpec(
                    name = "spec with inputs",
                    inputs = ActTestSpecInput(eventFile.toPath(), emptyMap())
                )

                "secrets" -> actSpec(
                    name = "spec with inputs",
                    secrets = ActTestSpecInput(eventFile.toPath(), emptyMap())
                )
