...
fun assertEventsPayload(report: File, specs: ActTestReportSpecsBuilder.() -> Unit) =
    parseSpecs(
        report,
...
fun assertEventsWithOutput(report: File, specs: ActTestReportSpecsBuilder.() -> Unit) =
    parseSpecs(
        report,
        ParseOption.INCLUDE_TEST_OUTPUT
    ) shouldBeEqual ActTestReportSpecsBuilder().apply(specs).build()

fun assertEvents(report: File, specs: ActTestReportSpecsBuilder.() -> Unit) =
...
class ActTestReportJobsBuilder(private val specName: String) {
    ...
    fun job(id: Long? = null, name: String, result: Status, output: String? = null) =
